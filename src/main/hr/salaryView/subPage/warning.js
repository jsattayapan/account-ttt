...
import { getWarningById, updateWarningApprove } from '../../tunnel'
              ...
              <tbody>
                  ...
                  warningList.sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp)).map(warning =>
                    ...
                    <td>{warning.remark}</td>
                      <td>{warning.accepted === null ? <label name={warning.id} for={warning.id} className="btn btn-info" >บันทึกรับทราบ</label> : <button className="btn btn-link" ><a href={IP +'/public/warning-approve/'+ warning.accepted} target="_blank" download>ดูใบบันทึก</a></button>}
