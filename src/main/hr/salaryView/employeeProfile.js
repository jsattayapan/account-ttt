...
import Notes from './subPage/notes'
 ...
 const Profile = props => {
  ...
  const empInfoSection = (
    ...
    />
  <TextField title='ลาป่วยคงเหลือ: ' value={employeeInfo.remainSickLeaveDay}  icon={<FaSyringe />}
    />
  <TextField title='ลาพักร้อนคงเหลือ: ' value={employeeInfo.remainYearlyLeaveDay}  icon={<FaSnowman />}
      />
    <TextField title='ลา Extra คงเหลือ: ' value={employeeInfo.publicHolidayList.length}  icon={<FaSnowman />}
          />
    ...
    <div className="row">
      ...
      <div className="col-12">
        <div className="row">
          ...
          {
            subPage === 'ตารางเวลา' ? <Timetable employeeInfo={employeeInfo} refreshEmployeeList={props.refreshEmployeeList} user={props.user} /> : ''
          }
